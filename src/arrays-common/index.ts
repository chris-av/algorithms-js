...
type ArrayofArrays = Array<number[]>

function vanillaMethod(arrayOfArrays: ArrayofArrays) {
  let accum = [];
...
export default function reduceMethod(arrayOfArrays: ArrayofArrays) {
  return arrayOfArrays.reduce((accum, curr) => {
    if (accum.length === 0) { accum = curr; }
    return accum.filter(a => curr.indexOf(a) >= 0);
  });
}
